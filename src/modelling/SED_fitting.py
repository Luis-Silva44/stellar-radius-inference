from src.data_retrieval.auxiliary_functions import band_wavelen, flux_unit_change
from src.data_retrieval.gaia_module import gaia_values
from src.data_retrieval.wise_module import wise_values 
from src.data_retrieval.two_mass_module import two_mass_values 
from src.modelling.SED_flux import SED_bands, SED_attenuated
from src.analysis.graphs_visualization import SED_fitting_plot
...
def SED_fitting(star_name, Teff, metallicity, log_g, Ebv, unit):
    ...
    SED_flux_Jy = SED_bands(filter_wavelen, Teff, metallicity, log_g, Ebv)
    ...
    SED_wavelen, SED_att = SED_attenuated(Teff,metallicity,log_g,Ebv)
...
def single_star_tester(star_name, Teff, metallicity, logg, Ebv, table_value, unit):
    radius, _ = SED_fitting(star_name, Teff, metallicity, logg, Ebv, unit)
...
def star_set_tester(star_list, unit):
    ...
    for i in range(len(star_list)):
        ...
        try:
            radius, _ = SED_fitting(star_name, Teff, metallicity, log_g, Ebv, unit)
